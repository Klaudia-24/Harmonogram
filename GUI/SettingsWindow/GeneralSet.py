...
class GeneralSet(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.colourPalettesDict = dict()
        self.colourPalettesDict = readColourPalettesFromYamlFile()
        # logger.debug(self.colourPalettesDict)
        self.generalSet.navBtnStyleComboBox.activated.connect(self.setButtonsPreview)
        self.keysList = []
        for key in self.colourPalettesDict[0]:
    ...
    def setCurrentData(self, settingsDict):
        ...
        for key in self.colourPalettesDict[0]:
            self.generalSet.navBtnStyleComboBox.addItem(self.colourPalettesDict[0][key]['name'])
            # self.keysList.append(key)
    ...
    def setButtonsPreview(self):
        currentIndex = self.generalSet.navBtnStyleComboBox.currentIndex()
        if currentIndex == -1:
            currentIndex = 0

        currentKey = self.keysList[currentIndex]

        # logger.debug(currentKey)
        self.generalSet.dayColourBtn.setStyleSheet(f"background-color:{self.colourPalettesDict[0][currentKey]['colour1']}; "
                                                   f"color:{self.colourPalettesDict[0][currentKey]['fontColour1']}")
        self.generalSet.weekColourBtn.setStyleSheet(f"background-color:{self.colourPalettesDict[0][currentKey]['colour2']}; "
                                                    f"color:{self.colourPalettesDict[0][currentKey]['fontColour2']}")
        self.generalSet.monthColourBtn.setStyleSheet(f"background-color:{self.colourPalettesDict[0][currentKey]['colour3']};"
                                                     f"color:{self.colourPalettesDict[0][currentKey]['fontColour3']}")
        self.generalSet.newEventColourBtn.setStyleSheet(f"background-color:{self.colourPalettesDict[0][currentKey]['colour4']};"
                                                        f"color:{self.colourPalettesDict[0][currentKey]['fontColour4']}")
